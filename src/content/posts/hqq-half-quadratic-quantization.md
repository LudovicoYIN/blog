...
这张图也解释了 HQQ 为什么方便接入推理 kernel：反量化最终只有减零点、乘尺度两步，是线性的。无论 kernel 选择先展开再 GEMM，还是把它融入矩阵乘，数学形式都很普通。
...
这就是标准 affine min-max quantization。分母太小时代码会把 $s$ 设为 `1.0`，随后将它限制在 `2e4` 以下，避免半精度下出现数值问题。至此其实已经可以量化；但纯 min-max 的问题也很熟悉：它只保证范围覆盖，不保证 16 个离散格子的位置对这组真实权重最合适。
...
直觉上，$e$ 把残差里不该强行拟合的部分软阈值化；零点再根据其余误差整体平移。这样做不会像逐元素贪心那样反复改动每个整数码，而是用一个组共享的 $z$ 来移动整套码本。计算量小，因而可以很快扫完整个模型。
