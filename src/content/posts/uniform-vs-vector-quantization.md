...
## 为什么 EfficientQAT 仍然选择均匀量化

均匀量化并不一定在极低 bit 下精度最好，但它有明显的工程优势：

- 计算规则简单，容易映射到整数运算和现有 NPU/GPU kernel；
- `scale/offset` 的 metadata 结构简单；
- 可以自然扩展到权重和激活同时量化；
- 不需要在推理时维护复杂的码本查找路径。

EfficientQAT 的思路不是换成复杂的码本，而是在均匀量化的框架里训练更多参数：Block-AP 训练 block 内的原始权重和量化参数，E2E-QP 再端到端调整量化步长。这样做是为了在保留均匀量化部署优势的同时，减少 2-bit/3-bit 下的精度损失。
